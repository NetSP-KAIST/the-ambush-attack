...
CONCURR = True
BATCH_SIZE = 1 # 0: no partial gossip; 1: partial gossip; more than 1: batched gossip
GOSSIP_P = 0.1
NUM_WORKER = 50
...
##### file names #####
PING_DATASET_FILENAME = './data-ping/wp-simplified.csv'
...
def read_latency_file(rtt_filename):
    ...
    rtt_df = rtt_df[['source', 'destination', 'avg']]
    # ott stands for one-way trip time, 
    rtt_df['avg'] = rtt_df['avg'] / 2
...
def sample_locations(num_nodes, num_clients):
    all_nodes = random.sample(list(UNIQUE_SRCS), num_nodes)
...
def chk_input_and_setup_dir():
    ...
    parser.add_argument('--gossip_p', type=str, help='The probability of gossip. When \
                        p==1 all nodes must gossip everythin, when p==0, they don\'t \
                        gossip at all.', default=GOSSIP_P)
    parser.add_argument('--batch_size', type=str, help='The gossip batch size. When \
                        size==0, nodes do not gossip; when size==1, nodes do partial \
                        gossip; when size>1, nodes do batched gossip', default=BATCH_SIZE)
    parser.add_argument('--txs', type=str, help='One integer indicatiing the number of \
                        transactions. When this argument is provided, the code generates \
                        transaction orderings wth this number of benign user \
                        transactions inserted.', default = 300)

    args = parser.parse_args()
    num_nodes = args.num_nodes
    output_dir = args.output_dir
    GOSSIP_P = float(args.gossip_p)
    BATCH_SIZE = int(args.batch_size)
    num_txs = int(args.txs)
    ...
    return int(num_nodes), template_filename, reordered_template_filename, new_dir, num_txs
...
def concur_main():
    ...
    num_nodes, template_filename, reordered_template_filename, home_dir, num_txs = res
    ...
    inputs = []
    for i in range(50): 
        out_filename = os.path.join(home_dir, str(num_txs).zfill(4))
        out_filename = os.path.join(out_filename, str(i).zfill(2)+'.csv')
        inputs.append((num_txs, NUM_CLIENTS, num_nodes, adv_txs_template, adv_reorder_txs, out_filename, i))
...
def single_main():
    ...
    run(num_txs, NUM_CLIENTS, num_nodes, adv_txs_template, adv_reorder_txs, out_filename, 0)

...
// taeung
struct MsgGossipBlock {
    static const opcode_t opcode = 0x9;
    DataStream serialized;
    std::vector<uint256_t> cmd_hashes;

    // Constructor for sending
    MsgGossipBlock(const std::vector<uint256_t> &cmd_hashes) {
        serialized << htole((uint32_t)cmd_hashes.size());
        for (const auto &h : cmd_hashes) {
            serialized << h;
        }
    }

    // Constructor for receiving
    MsgGossipBlock(salticidae::DataStream &&s) {
        uint32_t n;
        s >> n;
        n = letoh(n);
        cmd_hashes.resize(n);
...
class HotStuffBase: public HotStuffCore {
    ...
    // taeung; changed private to protected
    protected:

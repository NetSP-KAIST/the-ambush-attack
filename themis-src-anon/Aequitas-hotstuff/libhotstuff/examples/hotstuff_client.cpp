...
bool try_send(bool check = true) {
    if ((!check || waiting.size() < max_async_num) && max_iter_num)
    {
        ...
        // HOTSTUFF_LOG_INFO("Waiting queue size after sending: %lu", waiting.size());
        if (max_iter_num > 0)
            max_iter_num--;
        return true;
    }
...
int main(int argc, char **argv) {
    ...
    auto opt_max_iter_num = Config::OptValInt::create(10000000);
    auto opt_max_async_num = Config::OptValInt::create(1000000);

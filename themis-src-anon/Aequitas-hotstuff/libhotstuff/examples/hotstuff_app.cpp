...
using hotstuff::MsgGossipCmd;
using hotstuff::MsgGossipBlock;
...
# define gossip_probability 0.05
# define gossip_queue_size 1
...
class HotStuffApp: public HotStuff {
    ...
    mutable std::mutex gossip_mtx;
    // unordered set tracks a record of gossiped commands.
    std::unordered_set<uint256_t> gossiped_cmds;
    std::unordered_map<PeerId, size_t> peer_id_to_index;
    std::vector<std::vector<uint256_t>> pending_cmds_for_gossip_block_per_replica;
    size_t gossiped_msg_sent = 0;
    size_t gossiped_msg_recv = 0;
    ...
    void gossip_block_handler(MsgGossipBlock &&msg, const Net::conn_t &conn);
...
void HotStuffApp::client_request_cmd_handler(MsgReqCmd &&msg, const conn_t &conn) {
    ...
    // Create a random generator and distribution (one instance is fine)
    ...
    {
        // Lock the mutex to protect gossiped_cmds & pending_cmds_for_gossip_block_per_replica
        std::lock_guard<std::mutex> lock(gossip_mtx);

        // Check if we already know this command
        if (gossiped_cmds.find(cmd_hash) == gossiped_cmds.end()) {
            // Mark the command as gossiped
            gossiped_cmds.insert(cmd_hash);
            is_new = true;

            // Gossip logic
            for (const auto &peer_id : peers) {
                // Don't gossip to self
                if (peer_id == pn.get_peer_id()) 
                    continue;

                // Decide if we should gossip to this peer
                double r = dis(gen);
                if (r < gossip_probability) {
                    size_t idx = peer_id_to_index.at(peer_id);
                    pending_cmds_for_gossip_block_per_replica[idx].push_back(cmd_hash);

                    // Send a gossip block if the transaction gossip queue reached the block size (or group size)
                    if (pending_cmds_for_gossip_block_per_replica[idx].size() >= gossip_queue_size) {
                        MsgGossipBlock gossip_block(pending_cmds_for_gossip_block_per_replica[idx]);
                        pn.send_msg(gossip_block, peer_id);
                        gossiped_msg_sent++;
                        pending_cmds_for_gossip_block_per_replica[idx].clear();
                    }
                }
            }
        }
...
void HotStuffApp::gossip_block_handler(MsgGossipBlock &&msg, const Net::conn_t &conn) {
    HOTSTUFF_LOG_DEBUG("received gossip block from %s", std::string(conn->get_peer_addr()).c_str());
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_real_distribution<> dis(0.0, 1.0);
   
    {
        ...
        // For each cmd in the received gossip block
        for (auto &cmd_hash : msg.cmd_hashes) {
            if (gossiped_cmds.find(cmd_hash) == gossiped_cmds.end()) {
                gossiped_cmds.insert(cmd_hash);
                for (const auto &peer_id : peers) {
                    if (peer_id == pn.get_peer_id()) 
...
void HotStuffApp::start(const std::vector<std::tuple<NetAddr, bytearray_t, bytearray_t>> &reps) {
    ...
    pn.reg_handler(salticidae::generic_bind(&HotStuffApp::gossip_block_handler, this, _1, _2));

    TimerEvent check_conn_timer(ec, [this](TimerEvent &event) {
        size_t num_connections = 0;
        for (const auto &peer_id : peers) {
            auto conn = pn.get_peer_conn(peer_id);
            if (conn != nullptr && !conn->is_terminated()) {
                num_connections++;
            }
        }
        HOTSTUFF_LOG_DEBUG("Number of connected peers: %zu", num_connections);
        HOTSTUFF_LOG_DEBUG("Number of replicas: %zu", peers.size());

        // Reschedule the timer
        event.add(10.0); // Check every 5 seconds
    });

    // // Start the timer
    check_conn_timer.add(5.0);

    // map PeerId to index
    pending_cmds_for_gossip_block_per_replica.resize(peers.size());
    peer_id_to_index.reserve(peers.size());
    for (size_t i = 0; i < peers.size(); i++) {
        peer_id_to_index[peers[i]] = i;
    }
...
void HotStuffApp::print_stat() const {
    ...
    HOTSTUFF_LOG_INFO("--- Gossip msg. (10s) ---");
    HOTSTUFF_LOG_INFO("Gossiped messages sent: %zu", gossiped_msg_sent);
    HOTSTUFF_LOG_INFO("Gossiped messages received: %zu", gossiped_msg_recv);
    HOTSTUFF_LOG_INFO("--- end gossip msg. ---");
#endif
}